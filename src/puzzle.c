...
int coste(tEstado *e, unsigned op) { return 1; }
...
int piezasmalcolocadas(const tEstado *e) {
  ...
  for (int i = 0; i < DIMENSION; ++i)
    for (int j = 0; j < DIMENSION; ++j)
      if (e->celdas[i][j] != obj->celdas[i][j]) ++cont;
  return cont;
}

int Heuristica(const tEstado *e, Heuristicas h) { return h(e); }

int heuristica(tEstado *e) { return Heuristica(e, manhattan); }

void dispEstado(tEstado *estado) {
  for (int i = 0; i < DIMENSION; ++i) {
    printf("[");
    for (int j = 0; j < DIMENSION; ++j) printf("%d", estado->celdas[i][j]);
    printf("]\n");
  }
}

void dispOperador(unsigned op) { printf("%s\n\n", operadores[op - 1]); }
...
int esObjetivo(tEstado *estado) { return iguales(estado, estadoObjetivo()); }
...
static void intercambioEstado(tEstado *e, int fnew, int cnew, int fold,
                              int cold) {
...
tEstado *aplicaOperador(tEstado *estado, unsigned op) {
  ...
  intercambioEstado(nuevo, nuevo->filas[0], nuevo->cols[0], estado->filas[0],
                    estado->cols[0]);
  return nuevo;
}